...
import type { ChatState } from '@/types/stores/stores';
    ...
    (set, get) => ({
      ...
      fetchMessages: async (conversationId) => {
        ...
        try {
          ...
          set((state) => {
            const previous = state.messages[convoId]?.items ?? [];
            const merged =
              previous.length > 0 ? [...processed, ...previous] : processed;
      ...
      sendGroupMessage: async (conversationId, content, imgUrl) => {
        try {
          ...
          console.error('Lỗi xảy ra khi send group message', error);
        }
      },
